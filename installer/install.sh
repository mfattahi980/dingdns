...
#
# DingDns Installer
# Usage: curl -sSL https://get.dingdns.com/install.sh | bash
#
...
REPO_URL="https://github.com/dingdns/dingdns"
GO_VERSION="1.22.5"
...
build_backend() {
    ...
    /usr/local/go/bin/go mod tidy 2>&1 | tail -1 || true
    ...
    cat > "${CONFIG_FILE}" <<EOF
{
  ...
  "default_ttl": 300,
  "ns_primary": "ns1.dingdns.com",
  "ns_secondary": "ns2.dingdns.com",
  "admin_email": "admin@dingdns.com",
  "data_dir": "${DATA_DIR}"
}
EOF
...
setup_firewall() {
    ...
    # Ensure ufw is installed (already done in install_dependencies)
    # Enable ufw if not active, but allow SSH first to avoid lockout
    if command -v ufw &>/dev/null; then
        # Allow SSH before enabling to avoid lockout
        ufw allow 22/tcp  > /dev/null 2>&1 || true
        ufw allow 53/tcp  > /dev/null 2>&1
        ufw allow 53/udp  > /dev/null 2>&1
        ufw allow 80/tcp  > /dev/null 2>&1   # Let's Encrypt HTTP challenge
        ufw allow 443/tcp > /dev/null 2>&1   # HTTPS
        ufw allow 8080/tcp > /dev/null 2>&1  # Admin panel / API
